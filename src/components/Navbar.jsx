...
const Navbar = () => {
    ...
    return (
        <div className='fixed z-[999] w-full px-4 md:px-20 py-5 flex justify-between bg-gray-100 opacity-90'>
            <div className="logo">
            ...
            <div className="links hidden md:flex items-center gap-10">
                {['Services', 'Our Work', 'About Us', 'Insights', 'Contact Us'].map((link, index) => (
                    <a key={index} href={`#`} className={`text-md font-[500] capitalize opacity-100 ${index === 4 && 'ml-32'}`}> {link} </a>
                ))}
            </div>
            <div className="flex md:hidden items-center">
                <button onClick={toggleMenu} className="text-md font-[500] capitalize opacity-100">Menu</button>
            </div>
            {menuOpen && (
                <div className="absolute top-16 right-4 bg-white shadow-lg rounded-lg w-48 py-2 flex flex-col items-start md:hidden">
                    {['Services', 'Our Work', 'About Us', 'Insights', 'Contact Us'].map((link, index) => (
                        <a key={index} href={`#`} className="text-md font-[500] capitalize py-2 px-4 w-full text-left hover:bg-gray-100"> {link} </a>
                    ))}
                </div>
            )}
        </div>
